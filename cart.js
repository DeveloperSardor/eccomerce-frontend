...
function renderProduct() {
  ...
  productsInCart.forEach((el, idx) => {
    ...
    leftDelProduct.addEventListener("click", () =>
    ...
    right.setAttribute(
      "class",
      "right w-40 d-flex align-items-center justify-content-around"
    );
    ...
    rightUnitPrice.innerHTML = `$${Math.round( el.price * el.count)}`;
    rightPrice.classList.add("item_p");
...
function renderCheckOut() {
  ...
  const allProductSum = getTotalFromObjectsArray(productsInCart);
